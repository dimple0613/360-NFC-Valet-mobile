...
function OfferImgCard({ offer, onOpen }) {
  ...
  return (
    <button type="button" className="offer-img-card" onClick={() => onOpen?.(offer)}>
      <div className="offer-img" style={{ background: grad }}>
        ...
        {list.map((o) => {
          ...
          return (
            <button key={o.id} type="button" className="list-card" onClick={() => onOpen(o)}>
              <div className="list-card-img" style={{ background: OFFER_GRADIENT[o.category] || "linear-gradient(135deg,#2A3C61,#6C7A93)" }}>
  ...
  const startNfcScan = async () => {
    ...
    try {
      ...
      ndef.addEventListener("reading", ({ serialNumber, message }) => {
        ...
        if (message && message.records) {
          for (const rec of message.records) {
            if (rec.recordType === "text" && rec.data) {
              ...
              const text = decoder.decode(rec.data).trim();
        ...
        if (!uid) {
          const cleaned = (serialNumber || "").replace(/:/g, "");
          const digits = cleaned.replace(/[^0-9]/g, "");
          if (/^\d{4,6}$/.test(digits)) uid = digits;
          else {
            const hexMatch = cleaned.match(/[0-9A-Fa-f]{8,}/);
            if (hexMatch) {
              const dec = parseInt(hexMatch[0], 16).toString();
              if (/^\d{4,6}$/.test(dec)) uid = dec;
            }
          }
        }
      ...
      setNfcScanning(false);
      setNfcError(e.message?.includes("permission")
...
function TapApp() {
  ...
  useEffect(() => {
    ...
    setError("");
    api(`/public/tap/${encodeURIComponent(uid)}`)
      .then((d) => {
        if (!alive) return;
        setData(d);
  ...
  const onSubmitEta = async (minutes) => {
    ...
    setEtaError("");
    try {
      const res = await api(`/public/tap/${encodeURIComponent(uid)}`, {
        method: "POST",
        body: { minutes },
      });
      if (mounted.current) {
